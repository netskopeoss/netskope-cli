...
**Output size tip:** API responses can be large (100+ records, 500KB+). Always use `--limit`, plus `--api-fields` (server-side, events/alerts/incidents) or the global `--fields`/`-f` (client-side, any command), to keep output manageable. Start with `--limit 5 -o json` for exploration, then increase as needed.
...
1. **Not all response fields are valid `--api-fields` values.** Some fields that appear in the full response (e.g. `dlp_file`, `dlp_match_info` for incidents) are **not accepted** by the API as projection fields and will cause a 400 error.
2. **If you get a 400 "unrecognized field" error**, drop the offending field from `--api-fields` and retry, or omit `--api-fields` and select client-side with `--fields` (or `jq`).
3. **Safe common `--api-fields` values** that work across most event types: `user`, `app`, `activity`, `object`, `timestamp`, `severity`, `srcip`, `action`, `incident_id`, `alert_name`, `alert_type`

**Pattern:** trim server-side with `--api-fields`, then shape client-side with `--fields`/`--where`/`--sort`:
```bash
ntsk incidents list --start 7d --limit 10 --api-fields incident_id,user,app,severity --where 'severity eq "High"' -o json
```
...
ntsk alerts list --start 7d --api-fields alert_name,alert_type,severity,app,timestamp -o json
...
**Safe `--api-fields` values for alerts:** `alert_name`, `alert_type`, `severity`, `app`, `user`, `action`, `activity`, `timestamp`, `cci`, `ccl`, `policy`, `status`
...
- `--api-fields field1,field2` — server-side field projection; the global `--fields`/`-f` picks columns client-side (see the `--fields` vs `--api-fields` section above)
...
ntsk events application --start 7d --query 'app eq "Slack"' --api-fields user,activity,app,timestamp
ntsk events network --start 1h --api-fields srcip,dstip,app,action,dstport
...
ntsk incidents list --api-fields incident_id,user,app,object,severity,timestamp --limit 10 -o json
...
**Safe `--api-fields` values for incidents:** `incident_id`, `user`, `app`, `object`, `severity`, `timestamp`, `srcip`, `status`, `domain`, `activity`
...
- **Column selection** is the global `--fields` (client-side, any response field, nested paths). AICC endpoints have no server-side projection.
