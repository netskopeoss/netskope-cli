...
- Homebrew installs the sdist with pip's `--no-binary=:all:`, which also builds the build backend from source. hatchling is pure Python so that is quick; do not move to a compiled backend (uv_build, maturin) without re-testing the formula. Verify with `HOMEBREW_NO_AUTO_UPDATE=1 brew install --build-from-source netskopeoss/tap/netskope` before announcing, and do it *before* pushing the formula by copying it over the tapped file at `$(brew --repository netskopeoss/tap)/Formula/netskope.rb` (Homebrew refuses a loose formula path). Same day is fine: Homebrew's `std_pip_args` does pass `--uploaded-prior-to=P1D`, but `Language::Python::Virtualenv#pip_install` stages each resource first (Homebrew downloads the pinned URL and checks the sha256 itself) and runs pip against a local path, so there is no index query for that flag to filter. Verified on 1.5.0 against an sdist uploaded minutes earlier.
  - Note: `HOMEBREW_NO_AUTO_UPDATE=1` is not optional. Copying the formula over the tapped file leaves the tap's
    git working tree dirty, and an auto-update stashes that edit, fast-forwards the tap, then pops the stash onto
    the new HEAD. On 1.5.1 the pop conflicted and left `<<<<<<<` markers in `Formula/netskope.rb`, so the install
