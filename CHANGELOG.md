...
- `--exact` reads pages in whatever order the endpoint returns them, so a row indexed between two requests can be counted twice or missed; `--exact --help` now says the total is exact to within the tenant's ingest lag. The existing guard still errors outright on an endpoint that ignores `offset`.
