...
- GitHub Actions: `ci.yml` runs `uv sync --locked`, `ruff check`, `ruff format --check`, `ty check` and `pytest` on Python 3.11 and 3.14 for every pull request and push to master, then builds the wheel and sdist and installs each into a clean environment to check the CLI runs and its command tree is intact (`scripts/smoke-dist.sh`, the check that would have caught v1.4.6). `release.yml` publishes to PyPI from a `vX.Y.Z` tag through PyPI's Trusted Publisher after checking that the tag is on master and matches the project version, and runs the same smoke test on the artifacts it uploads. Its header documents the GitHub half of the setup as well as the pypi.org half: an environment referenced by a workflow is created on first use with no protection rules, so the `pypi` environment needs required reviewers and a `v*` tag rule before the first release. Both workflows pin every action by commit SHA.
...
- `--count` on the datasearch commands and on `events infrastructure` fetches the full 10,000-row page (`events audit` states a total, and `events transaction` returns aggregate metrics rather than rows, so both count with `--limit`), and a page that came back full is a lower bound whatever filled it, the 10,000-row cap or a smaller `--limit`: on a terminal, table/human output prints `10000+` with a stderr notice; `-o json`/`jsonl`/`csv`/`yaml` and any piped output print the bare integer so `$(ntsk ... --count)` keeps parsing a number, with the same notice on stderr; a listing whose page came back full says so in its banner (`10,000+ results (capped)`), which is decided per page rather than only under `--count`; `alerts summary` notes when its aggregation covered only the first 10,000 alerts. One rule decides, shared with `ntsk status`: a stated envelope total (`total`, `totalResults`, `status.total`) is exact; a `status.count` larger than the rows of a full page is the lower bound; a `--count --where` over a full page is a lower bound whatever the total says. `--count` with `--group-by` reports the number of groups on every events command.
- A `--count` sends `fields=_id` when nothing reads the rows, so counting a full page moves kilobytes instead of the tens of megabytes of complete event documents it used to (hundreds under `--exact`, which reads up to 20 pages). `--where`, `--group-by` and an explicit `--api-fields` opt out, since each of those does read the rows.
- `npa policy rules get` prints the rule object rather than its `{"data": ..., "status": ...}` envelope.
- `typer` 0.25.1 to 0.27.2. The command-tree walkers introspect typer's public classes (`typer.core.TyperGroup`, `TyperCommand`, `TyperOption`, `TyperArgument`) instead of the standalone `click` package; `ntsk commands --json` keeps upper-case argument names and click's type names. Positional arguments render differently in help and error text: `Usage: netskope users update [OPTIONS] {user_id}` where 0.25 printed `USER_ID`, and `Error: Missing argument 'user_id'.` where it printed `'USER_ID'`. That is typer's own rendering and the CLI does not override it; `ntsk commands` keeps its `<APP_ID>` signature notation, which is a documented output format rather than an echo of the parser.
