...
- `--fields` naming a field that no returned record has now exits 2 with the close-match suggestions (previously a warning and blank columns). Only provably wrong names fail: a path whose parent is null or empty in every record (`host_info.os` when no device reported `host_info`, `tags[].name` when every `tags` is empty) warns and leaves the column blank. A hidden `_`-prefixed name is reported with a pointer at `--raw`. Only lists of records are strict: single-object responses (`get`/`create`/`update` envelopes), group-by rows, `<field>_iso` companions in table output and globs under a null parent all warn. Add the new global `--lenient` to get the old behaviour everywhere. A server-side `--api-fields` projection keeps the `<field>_iso` companions of the fields it names.
- Fix `--count` on `alerts list`, `events ...` and `incidents list`, and the event counts in `ntsk status`, reporting the datasearch page cap of 10,000 as if it were the total. A count that filled the page now prints `10000+` with a stderr notice, the result banner reads `10,000+ results (capped)`, and `ntsk status` renders `≥10,000` in the table and `alerts_capped: true` (and friends) in JSON. `events <type> --count` and `incidents list --count` now fetch the full page like `alerts list` did instead of counting only `--limit` rows. `alerts summary` notes when its aggregation covered only the first 10,000 alerts. Note for scripts: a capped `--count` prints `N+`, not an integer, in every output format including `-o json`; add `--exact` or handle the suffix. `--count` with `--group-by` now reports the number of groups on every events command (`events list --type ... --group-by ... --count` used to ignore the grouping and count events). An HTTP 200 `ok: 0` error envelope on `alerts list --count`, `incidents list --count` and `alerts summary` is now an error rather than a count of 1 or an empty summary, and its message is markup-safe on every events subcommand. A stated envelope total (`total`, `totalResults`, `status.total`) is always exact, `status.count` is consulted last, and a `--count --where` over a full page is a lower bound whatever the total says. The page-cap machinery applies to datasearch endpoints only; `events audit` and `infrastructure` count with `--limit` rows and the envelope total, as before. `ntsk status` still prints its "All API calls failed" hint when every metric is unavailable.
- Add the global `--exact` flag: with `--count` on events/alerts/incidents it pages through the endpoint with `offset` in 10,000-row steps for the true total, applying `--where` per page, up to `NETSKOPE_COUNT_CEILING` rows (default 200,000; prints `N+` if reached). Datasearch endpoints only: `events audit`, `infrastructure` and `transaction` count a single page and say so. `--where` sees group-by rows flattened, as the formatter does, and a filter path no row has is called out. The end of the time window is pinned before the first page so rows arriving mid-count cannot shift later pages, and `--group-by` results are counted on a single page. An HTTP 400 for a name that widening added to `--api-fields` says which option referenced it.
