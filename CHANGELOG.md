...
- A `--fields` name that no returned record has stays a warning with close-match suggestions and a blank (table/csv) or null (json/yaml) column; it never fails the command. Which keys a page carries depends on the rows that landed in it (optional attributes on fixed-schema endpoints such as `users list --fields externalId`, subtypes on events), so an exit code keyed on it would change from one page to the next and break scripts at random. The warning is specific where it can be: a hidden `_`-prefixed name points at `--raw`, and a `<field>_iso` name in table output says the companion exists only in json/jsonl/csv/yaml. A server-side `--api-fields` projection keeps the `<field>_iso` companions of the fields it names.
- Fix `--count` on `alerts list`, `events ...` and `incidents list`, and the event counts in `ntsk status`, reporting the datasearch page cap of 10,000 as if it were the total. A count that filled the page now prints `10000+` with a stderr notice, the result banner reads `10,000+ results (capped)`, and `ntsk status` renders `≥10,000` in the table and `alerts_capped: true` (and friends) in JSON. `events <type> --count` and `incidents list --count` now fetch the full page like `alerts list` did instead of counting only `--limit` rows. `alerts summary` notes when its aggregation covered only the first 10,000 alerts. The `N+` marker is for people: `-o json`, `jsonl`, `csv` and `yaml`, and any output that is not a terminal, print the bare integer (a lower bound) so `$(ntsk ... --count)` keeps parsing a number, with the notice on stderr. `--count` with `--group-by` now reports the number of groups on every events command (`events list --type ... --group-by ... --count` used to ignore the grouping and count events). An HTTP 200 `ok: 0` error envelope on `alerts list --count`, `incidents list --count` and `alerts summary` is now an error rather than a count of 1 or an empty summary, and its message is markup-safe on every events subcommand. A stated envelope total (`total`, `totalResults`, `status.total`) is always exact; a `status.count` larger than the rows of a full page is reported as the lower bound (`N+`, `≥N`) by `--count` and `ntsk status` alike; and a `--count --where` over a full page is a lower bound whatever the total says. `events infrastructure` and `transaction` fetch the full page under `--count` too, and their capped notice says `--exact` cannot page them; `events audit` states a total, so it counts with `--limit` rows as before. `--count` no longer prints the `N+ results (capped)` listing banner on top of its own notice, and `-q` silences that banner on listings. `ntsk status` still prints its "All API calls failed" hint when every metric is unavailable.
- Add the global `--exact` flag: with `--count` on events/alerts/incidents it pages through the endpoint with `offset` in 10,000-row steps for the true total, applying `--where` per page, up to `NETSKOPE_COUNT_CEILING` rows (default 200,000; prints `N+` if reached). Datasearch endpoints only: `events audit`, `infrastructure` and `transaction` count a single page and say so. `--where` sees group-by rows flattened, as the formatter does, and a filter path no row has is called out. `--exact` requires `--start` (exit 2 otherwise): the commands resolve `--start`/`--end` to fixed epochs once, so every page is counted against the same window, whereas paging the API's rolling default window would count a moving target. `--group-by` results are counted on a single page. An endpoint that ignores `offset` (the same first `_id` on consecutive pages) is an error rather than a doubled or falsely exact count, and an `--api-fields` projection is widened with `_id` while counting so that check always has something to compare. An HTTP 400 for a name that widening added to `--api-fields` says which option referenced it, on `--exact` and the NPA `--api-fields` commands too.
...
- The single-page notices `--exact` prints off datasearch endpoints respect `--quiet`, the events title no longer repeats the total the result banner states, and `--verbose` metadata omits `ok`/`message`.
